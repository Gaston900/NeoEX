...
DEFINE_DEVICE_TYPE(NEOGEO_EK_GSC_CART, neogeo_ek_gsc_cart_device, "neocart_ek_gsc", "EK GSC Neo Geo Cart")

neogeo_ek_gsc_cart_device::neogeo_ek_gsc_cart_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock) :
	neogeo_rom_device(mconfig, NEOGEO_EK_GSC_CART, tag, owner, clock),
	m_nvram(*this, "nvram")
{
}

void neogeo_ek_gsc_cart_device::device_start()
{
...
void neogeo_ek_gsc_cart_device::device_add_mconfig(machine_config &config)
{
	neogeo_rom_device::device_add_mconfig(config);
	NVRAM(config, m_nvram);
}
