...
/*******************************************************************************************************************************************************************************************/
	...
	{"NeoGeo",          "neogeo", 	         FOLDER_NEOGEO,    IDI_FP_NEOGEO,	 0,			     0, 		 0,          0, CreateNEOGEOFolders },
    {"Capcom",		    "Capcom",		     FOLDER_CAPCOM,	   IDI_FP_CAPCOM,    0,			     0,          0,	         0, CreateCAPCOMFolders },
...
void CreateNEOGEOFolders(int parent_index)
{
	...
	LPTREEFOLDER lpFolder = treeFolders[parent_index];
    LPTREEFOLDER lpNeoGeoBT, lpNeoGeoCD, lpNeoGeoDS, lpNeoGeoDC, lpNeoGeoDM, lpNeoGeoEL, lpNeoGeoHC, lpNeoGeoST, lpNeoGeoHB, lpNeoGeoMG, lpNeoSD, lpGnGeo, lpNeoCD, lpNeoCDHB, lpNeoCDHC;
	...
	lpNeoGeoHB = NewFolder("NeoGeo HB", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
    ...
    lpGnGeo = NewFolder("GnGeo", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
    lpNeoCD = NewFolder("Neo CD", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
    lpNeoCDHB = NewFolder("Neo CD HB", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
    lpNeoCDHC = NewFolder("Neo CD Hack", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
	...
	for (jj = 0; jj < nGames; jj++)
	{
        ...
        // GnGeo
		if (!strcmp("3countbgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("2020bbgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("2020bbagno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("2020bbhgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("alpham2gno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("alpham2pgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("androdungno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("aodkgno", s))		   	        AddGame(lpGnGeo, jj);
		if (!strcmp("aofgno", s))	                AddGame(lpGnGeo, jj);
		if (!strcmp("aof2gno", s))			        AddGame(lpGnGeo, jj);
		if (!strcmp("aof2agno", s))			        AddGame(lpGnGeo, jj);
		if (!strcmp("aof3gno", s))			        AddGame(lpGnGeo, jj);
		if (!strcmp("aof3kgno", s))			        AddGame(lpGnGeo, jj);
		if (!strcmp("b2bgno", s))			        AddGame(lpGnGeo, jj);
		if (!strcmp("bakatonogno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("bangbeadgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("bjourneygno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("bjourneyhgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("blazstargno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("breakersgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("breakrevgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("bstarsgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("bstarshgno", s))		   	    AddGame(lpGnGeo, jj);
		if (!strcmp("bstars2gno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("burningfgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("burningfhgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("burningfpgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("burningfpagno", s))	        AddGame(lpGnGeo, jj);
		if (!strcmp("crswd2blgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("crswordgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("ct2k3sagno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("ct2k3spgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("cthd2003gno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("ctomadaygno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("cyberlipgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("diggermagno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("doubledrgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("dragonshgno", s))		   	    AddGame(lpGnGeo, jj);
		if (!strcmp("eightmangno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("fatfurspgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("fatfurspagno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("fatfury1gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("fatfury2gno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("fatfury3gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("fbfrenzygno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("fightfevgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("fightfevagno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("flipshotgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("froman2bgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("fswordsgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("galaxyfggno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("ganryugno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("garougno", s))		   	        AddGame(lpGnGeo, jj);
		if (!strcmp("garoublgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("garouhgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("garouhagno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("garoupgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("ghostlopgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("goalx3gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("gowcaizrgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("gpilotsgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("gpilotshgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("gururingno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("ironclagno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("ironcladgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("irrmazegno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("janshingno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("jockeygpgno", s))		   	    AddGame(lpGnGeo, jj);
		if (!strcmp("jockeygpagno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("joyjoygno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kabukiklgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("karnovrgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kf2k2mpgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("kf2k2mp2gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kf2k2plsgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kf2k2plagno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("kf2k3blgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kf2k3blagno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("kf2k3uplgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("kf2k3plgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("kf2k3pcbgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("kizunagno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("kof2000gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kof2000ngno", s))		   	    AddGame(lpGnGeo, jj);
		if (!strcmp("kof2001gno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("kof2001hgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kof2002gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kof2002bgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kof2003gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kof2003hgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kof10thgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kf10thepgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("kf2k5unigno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kof2k4segno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("kof94gno", s))	                AddGame(lpGnGeo, jj);
		if (!strcmp("kof95gno", s))	                AddGame(lpGnGeo, jj);
		if (!strcmp("kof95hgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("kof96gno", s))			        AddGame(lpGnGeo, jj);
		if (!strcmp("kof96hgno", s))		   	    AddGame(lpGnGeo, jj);
		if (!strcmp("kof97gno", s))	                AddGame(lpGnGeo, jj);
		if (!strcmp("kof97hgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kof97kgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kof97plsgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kof97orogno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kof98gno", s))			        AddGame(lpGnGeo, jj);
		if (!strcmp("kof98hgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kof98kgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("kof98kagno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kof99gno", s))	                AddGame(lpGnGeo, jj);
		if (!strcmp("kof99egno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("kof99hgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("kof99kgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("kof99kagno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kof99pgno", s))		   	    AddGame(lpGnGeo, jj);
		if (!strcmp("koggno", s))	                AddGame(lpGnGeo, jj);
		if (!strcmp("kotmgno", s))			        AddGame(lpGnGeo, jj);
		if (!strcmp("kotmhgno", s))			        AddGame(lpGnGeo, jj);
		if (!strcmp("kotm2gno", s))			        AddGame(lpGnGeo, jj);
		if (!strcmp("kotm2agno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("kotm2pgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("lans2004gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("lastbladgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("lastbladhgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("lastsolgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("lasthopegno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("lastbld2gno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("lbowlinggno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("legendosgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("lresortgno", s))		   	    AddGame(lpGnGeo, jj);
		if (!strcmp("lresortpgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("magdrop2gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("magdrop3gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("maglordgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("maglordhgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("mahretsugno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("marukodqgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("matrimgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("miexchnggno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("minasangno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("moshougigno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("ms5pcbgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("ms4plusgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("ms5plusgno", s))		   	    AddGame(lpGnGeo, jj);
		if (!strcmp("msluggno", s))	                AddGame(lpGnGeo, jj);
		if (!strcmp("mslug2gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("mslug2tgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("mslug3gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("mslug3hgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("mslug3b6gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("mslug4gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("mslug4hgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("mslug5gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("mslug5hgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("mslugxgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("mutnatgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("nam1975gno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("ncombatgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("ncombathgno", s))		   	    AddGame(lpGnGeo, jj);
		if (!strcmp("ncommandgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("neobombegno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("neocup98gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("neodriftgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("neomrdogno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("ninjamasgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("nitdgno", s))			        AddGame(lpGnGeo, jj);
		if (!strcmp("nitdblgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("overtopgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("panicbomgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("pbobbl2ngno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("pbobblengno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("pbobblenbgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("pgoalgno", s))			        AddGame(lpGnGeo, jj);
		if (!strcmp("pnyaagno", s))		   	        AddGame(lpGnGeo, jj);
		if (!strcmp("pnyaaagno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("popbouncgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("preisle2gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("pspikes2gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("pulstargno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("puzzldprgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("puzzledpgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("quizdai2gno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("quizdaisgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("quizdaiskgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("quizkofgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("quizkofkgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("ragnagrgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("rbff1gno", s))			        AddGame(lpGnGeo, jj);
		if (!strcmp("rbff1agno", s))		   	    AddGame(lpGnGeo, jj);
		if (!strcmp("rbff1kgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("rbff2gno", s))			        AddGame(lpGnGeo, jj);
		if (!strcmp("rbff2hgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("rbff2kgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("rbffspecgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("rbffspeckgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("ridherogno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("ridherohgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("roboarmygno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("roboarmyagno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("rotdgno", s))	                AddGame(lpGnGeo, jj);
		if (!strcmp("rotdhgno", s))	                AddGame(lpGnGeo, jj);
		if (!strcmp("s1945pgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("samsh5spgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("samsh5sphgno", s))		   	    AddGame(lpGnGeo, jj);
		if (!strcmp("samsh5sphogno", s))	        AddGame(lpGnGeo, jj);
		if (!strcmp("samshogno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("samshohgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("samsho2gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("samsho2kgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("samsho2kagno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("samsho3gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("samsho3hgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("samsho4gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("samsho4kgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("samsho5gno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("samsho5hgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("samsho5bgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("savageregno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("sbpgno", s))		   	        AddGame(lpGnGeo, jj);
		if (!strcmp("sdodgebgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("sengokugno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("sengokuhgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("sengoku2gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("sengoku3gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("sengoku3agno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("shocktr2gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("shocktrogno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("shocktroagno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("socbrawlgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("socbrawlhgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("sonicwi2gno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("sonicwi3gno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("spinmastgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("ssidekigno", s))		   	    AddGame(lpGnGeo, jj);
		if (!strcmp("ssideki2gno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("ssideki3gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("ssideki4gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("stakwingno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("stakwin2gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("strhoopgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("superspygno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("svcgno", s))		            AddGame(lpGnGeo, jj);
		if (!strcmp("svcbootgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("svcplusgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("svcplusagno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("svcsplusgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("svcpcbgno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("svcpcbagno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("tophuntrgno", s))		   	    AddGame(lpGnGeo, jj);
		if (!strcmp("tophuntrhgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("tpgolfgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("trallygno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("turfmastgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("twinsprigno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("twsoc96gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("viewpoingno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("vlinergno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("vliner7egno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("vliner6egno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("vliner54gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("vliner53gno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("wakuwak7gno", s))		        AddGame(lpGnGeo, jj);
		if (!strcmp("wh1gno", s))			        AddGame(lpGnGeo, jj);
		if (!strcmp("wh1hgno", s))	                AddGame(lpGnGeo, jj);
		if (!strcmp("wh1hagno", s))	                AddGame(lpGnGeo, jj);
		if (!strcmp("wh2gno", s))	                AddGame(lpGnGeo, jj);
		if (!strcmp("wh2hgno", s))		            AddGame(lpGnGeo, jj);
		if (!strcmp("wh2jgno", s))			        AddGame(lpGnGeo, jj);
		if (!strcmp("whpgno", s))		   	        AddGame(lpGnGeo, jj);
		if (!strcmp("wjammersgno", s))	            AddGame(lpGnGeo, jj);
		if (!strcmp("zedbladegno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("zintrckbgno", s))			    AddGame(lpGnGeo, jj);
		if (!strcmp("zupapagno", s))			    AddGame(lpGnGeo, jj);

        // NeoGeo CD
		if (!strcmp("3countbncd", s))	            AddGame(lpNeoCD, jj);
		if (!strcmp("2020bbncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("adkworldncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("alpham2ncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("aodkncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("aofncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("aof2ncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("aof3ncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("aof3ancd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("b2bncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("bjourneyncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("breakersncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("bstarsncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("bstars2ncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("burningfncd", s))		        AddGame(lpNeoCD, jj);
		if (!strcmp("cpbarrelncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("crswordncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("crswd2blncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("cyberlipncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("doubledrncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("doubledrancd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("fatfurspncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("fatfury1ncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("fatfury2ncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("fatfury3ncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("fatfury3ancd", s))		        AddGame(lpNeoCD, jj);
		if (!strcmp("fbfrenzyncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("froman2bncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("galaxyfgncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("gowcaizrncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("gpilotsncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("ironcladncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("janshinncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("joyjoyncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("kabukiklncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("karnovrncd", s))		        AddGame(lpNeoCD, jj);
		if (!strcmp("kof94ncd", s))		            AddGame(lpNeoCD, jj);
		if (!strcmp("kof94ancd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("kof95ncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("kof95ancd", s))		        AddGame(lpNeoCD, jj);
		if (!strcmp("kof96ncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("kof96ancd", s))	            AddGame(lpNeoCD, jj);
		if (!strcmp("kof96colncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("kof97ncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("kof98ncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("kof99ncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("kotm2ncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("lastbladncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("lastbld2ncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("lasthopencd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("lbowlingncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("looptrisncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("looptrspncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("lresortncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("magdrop2ncd", s))		        AddGame(lpNeoCD, jj);
		if (!strcmp("maglordncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("mahretsuncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("moshougincd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("mslugncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("mslug2ncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("mutnatncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("nam1975ncd", s))	            AddGame(lpNeoCD, jj);
		if (!strcmp("ncombatncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("ncommandncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("neodriftncd", s))		        AddGame(lpNeoCD, jj);
		if (!strcmp("ngcdspecncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("ninjamasncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("overtopncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("pbobblenncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("pgoalncdncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("pspikes2ncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("pulstarncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("quizkofncd", s))		        AddGame(lpNeoCD, jj);
		if (!strcmp("ragnagrdncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("rbff1ncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("rbff2ncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("rbffspecncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("ridheroncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("roboarmyncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("samshoncd", s))		        AddGame(lpNeoCD, jj);
		if (!strcmp("samsho2ncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("samsho3ncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("samsho4ncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("samsprpgncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("savagerencd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("sengokuncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("sengoku2ncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("socbrawlncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("sonicwi2ncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("sonicwi3ncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("ssidekincd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("ssideki2ncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("ssideki3ncd", s))		        AddGame(lpNeoCD, jj);
		if (!strcmp("stakwinncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("strhoopncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("superspyncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("tophuntrncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("totcncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("tpgolfncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("trallyncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("turfmastncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("twinsprincd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("viewpoinncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("wh1ncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("wh2ncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("wh2jncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("whpncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("wjammersncd", s))			    AddGame(lpNeoCD, jj);
		if (!strcmp("xenoncd", s))			        AddGame(lpNeoCD, jj);
		if (!strcmp("zintrckbncd", s))			    AddGame(lpNeoCD, jj);

        // Neo CD/HACK
		if (!strcmp("ffury_cd", s))	                AddGame(lpNeoCDHC, jj);
		if (!strcmp("ssrpg_cd", s))		    	    AddGame(lpNeoCDHC, jj);

        // Neo CD/HOMEBREW
		if (!strcmp("alice_cd", s))	                AddGame(lpNeoCDHB, jj);
		if (!strcmp("chelnov_cd", s))			    AddGame(lpNeoCDHB, jj);
		if (!strcmp("diff_cd", s))			        AddGame(lpNeoCDHB, jj);
		if (!strcmp("eira_cd", s))			        AddGame(lpNeoCDHB, jj);
		if (!strcmp("fukkireta_cd", s))			    AddGame(lpNeoCDHB, jj);
		if (!strcmp("gaxe_cd", s))			        AddGame(lpNeoCDHB, jj);
		if (!strcmp("gaxearr_cd", s))			    AddGame(lpNeoCDHB, jj);
		if (!strcmp("natademo_cd", s))			    AddGame(lpNeoCDHB, jj);
		if (!strcmp("ngdark_cd", s))			    AddGame(lpNeoCDHB, jj);
		if (!strcmp("noneon_cd", s))			    AddGame(lpNeoCDHB, jj);
		if (!strcmp("ntris3d_cd", s))			    AddGame(lpNeoCDHB, jj);
		if (!strcmp("phoenix_cd", s))			    AddGame(lpNeoCDHB, jj);
		if (!strcmp("pong_cd", s))			        AddGame(lpNeoCDHB, jj);
		if (!strcmp("pow_cd", s))			        AddGame(lpNeoCDHB, jj);
		if (!strcmp("shinobiarr_cd", s))		    AddGame(lpNeoCDHB, jj);
		if (!strcmp("speedball_cd", s))			    AddGame(lpNeoCDHB, jj);
		if (!strcmp("subspecies_cd", s))			AddGame(lpNeoCDHB, jj);
		if (!strcmp("visualnovel_cd", s))			AddGame(lpNeoCDHB, jj);
	}
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("neogeocd.cpp", s))             AddGame(lpFolder, jj);
	}
}
...
void CreateCAPCOMFolders(int parent_index)
{
	...
	for (jj = 0; jj < nGames; jj++)
	{
        ...
        // Bootleg
		if (!strcmp("1942abl", s))		            AddGame(lpCps1BT, jj);
		if (!strcmp("1942p", s))		            AddGame(lpCps1BT, jj);
		if (!strcmp("1943b", s))		            AddGame(lpCps1BT, jj);
		if (!strcmp("1943bj", s))		            AddGame(lpCps1BT, jj);
		if (!strcmp("bioniccbl", s))		        AddGame(lpCps1BT, jj);
		if (!strcmp("bioniccbl2", s))		        AddGame(lpCps1BT, jj);
		if (!strcmp("blktigerb1", s))		        AddGame(lpCps1BT, jj);
		if (!strcmp("blktigerb2", s))		        AddGame(lpCps1BT, jj);
		if (!strcmp("blktigerb3", s))		        AddGame(lpCps1BT, jj);
		if (!strcmp("commandob", s))		        AddGame(lpCps1BT, jj);
		if (!strcmp("commandob2", s))		        AddGame(lpCps1BT, jj);
		if (!strcmp("commandob3", s))		        AddGame(lpCps1BT, jj);
		if (!strcmp("sinvasnb", s))		            AddGame(lpCps1BT, jj);
		if (!strcmp("mercenario", s))		        AddGame(lpCps1BT, jj);
		if (!strcmp("gngbl", s))		            AddGame(lpCps1BT, jj);
		if (!strcmp("gngblita", s))  		        AddGame(lpCps1BT, jj);
		if (!strcmp("gunsmokeb", s))		        AddGame(lpCps1BT, jj);
		if (!strcmp("lastduelb", s))		        AddGame(lpCps1BT, jj);
		if (!strcmp("lwingsb", s))   		        AddGame(lpCps1BT, jj);
		if (!strcmp("trojanb", s))	    	        AddGame(lpCps1BT, jj);
		if (!strcmp("buraikenb", s))		        AddGame(lpCps1BT, jj);
		if (!strcmp("pkladiesbl", s))		        AddGame(lpCps1BT, jj);
		if (!strcmp("pkladiesbl2", s))		        AddGame(lpCps1BT, jj);
		if (!strcmp("pangb", s))		            AddGame(lpCps1BT, jj);
		if (!strcmp("pangbold", s))		            AddGame(lpCps1BT, jj);
		if (!strcmp("pangba", s))		            AddGame(lpCps1BT, jj);
		if (!strcmp("pangb2", s))		            AddGame(lpCps1BT, jj);
		if (!strcmp("pangbb", s))		            AddGame(lpCps1BT, jj);
		if (!strcmp("pangbp", s))		            AddGame(lpCps1BT, jj);
		if (!strcmp("pangbc", s))		            AddGame(lpCps1BT, jj);
		if (!strcmp("spangbl", s))	    	        AddGame(lpCps1BT, jj);
		if (!strcmp("spangbl2", s))		            AddGame(lpCps1BT, jj);
		if (!strcmp("mstworld", s))	    	        AddGame(lpCps1BT, jj);
		if (!strcmp("mstworld2", s))		        AddGame(lpCps1BT, jj);
		if (!strcmp("blockbl", s))	    	        AddGame(lpCps1BT, jj);
		if (!strcmp("sfjbl", s))	    	        AddGame(lpCps1BT, jj);
		if (!strcmp("tigeroadb", s))		        AddGame(lpCps1BT, jj);
		if (!strcmp("f1dreamb", s))		            AddGame(lpCps1BT, jj);
		if (!strcmp("f1dreamba", s))		        AddGame(lpCps1BT, jj);
		if (!strcmp("mach9", s))	     	        AddGame(lpCps1BT, jj);
		...
		if (!strcmp("jojobajr2", s))			    AddGame(lpCps3, jj);
		if (!strcmp("jojobanr1", s))			    AddGame(lpCps3, jj);
		if (!strcmp("jojobaner1", s))			    AddGame(lpCps3, jj);
		if (!strcmp("jojobaner2", s))			    AddGame(lpCps3, jj);
		if (!strcmp("jojobanr2", s))			    AddGame(lpCps3, jj);
		if (!strcmp("jojobar2", s))		            AddGame(lpCps3, jj);
		...
		if (!strcmp("redearthn", s))			    AddGame(lpCps3, jj);
		if (!strcmp("redearthnr1", s))			    AddGame(lpCps3, jj);
		if (!strcmp("redearthr1", s))		        AddGame(lpCps3, jj);
		if (!strcmp("sfiii", s))			        AddGame(lpCps3, jj);
		if (!strcmp("sfiii2", s))	                AddGame(lpCps3, jj);
		if (!strcmp("sfiii2a", s))	                AddGame(lpCps3, jj);
		...
		if (!strcmp("sfiii3na", s))			        AddGame(lpCps3, jj);
		if (!strcmp("sfiii3nr1", s))			    AddGame(lpCps3, jj);
		if (!strcmp("sfiii3nar1", s))			    AddGame(lpCps3, jj);
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("bionicc.cpp", s))              AddGame(lpBionicc, jj);
	}
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("cbasebal.cpp", s))             AddGame(lpCbasebal, jj);
	}
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("commando.cpp", s))            AddGame(lpCommando, jj);
	}
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("egghunt.cpp", s))            AddGame(lpEgghunt, jj);
	}
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("gunsmoke.cpp", s))            AddGame(lpGunsmoke, jj);
	}
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("higemaru.cpp", s))            AddGame(lpHigemaru, jj);
	}
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("kenseim.cpp", s))             AddGame(lpKenseim, jj);
	}
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("lastduel.cpp", s))            AddGame(lpLastduel, jj);
	}
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("lwings.cpp", s))              AddGame(lpLwings, jj);
	}
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("sf.cpp", s))                  AddGame(lpSf, jj);
	}
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("sidearms.cpp", s))            AddGame(lpSidearms, jj);
	}
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("srumbler.cpp", s))            AddGame(lpSrumbler, jj);
	}
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("supduck.cpp", s))            AddGame(lpSupduck, jj);
	}
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("tigeroad.cpp", s))            AddGame(lpTigeroad, jj);
	}
	...
	for (jj = 0; jj < nGames; jj++)
	{
		...
		if (!strcmp("vulgus.cpp", s))             AddGame(lpVulgus, jj);
	}
